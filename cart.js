...
function ready() {
  ...
  for (let i = 0; i < qtyEle.length; i++) {
    ...
    everyqtyele.addEventListener("click", updateOrderQty);
  }

  //Quantity add & subtract
  let addbtn = document.getElementsByClassName("addBtn");
  let subtractBtn = document.getElementsByClassName("subtractBtn");
  for (let i = 0; i < addbtn.length; i++) {
    let button = addbtn[i];
    button.addEventListener("click", quantityPlus1);
  }

  for (let i = 0; i < subtractBtn.length; i++) {
    let button = subtractBtn[i];
    button.addEventListener("click", quantityMinus1);
  }
...
function quantityPlus1(event) {
  let clickin = event.target;
  let quantDiv = clickin.parentElement.parentElement;

  let quantP = quantDiv.getElementsByClassName("cart-item-qty")[0];
  //   quantP.innerText = 1;
  //console.log(quantP.innerHTML);
  quantP.innerText = parseInt(quantP.innerText) + 1;
  quantP.click();
  updateTotalPrice();
}

function quantityMinus1(event) {
  let clickin = event.target;
  let quantDiv = clickin.parentElement.parentElement;
...
function updateOrderQty(event) {
  let qtychange0 = event.target;
  let qtychange = parseInt(qtychange0.innerText);
  console.log("what");
  if (isNaN(qtychange) || qtychange <= 0) {
    qtychange0.innerText = 1;
  }
  updateTotalPrice();
}
...
function updateTotalPrice() {
  ...
  for (let i = 0; i < cartRows.length; i++) {
    ...
    let qty = cartRow.getElementsByClassName("cart-item-qty")[0].innerText;
    total += itemPriceValue * parseInt(qty);
  }
...
function sandwitchobj() {
  ...
  for (let i = 0; i < pname.length; i++) {
    ...
    if (productName == "طعمية") {
      ...
      cartroww.getElementsByClassName("cart-item-price")[0].innerText =
        tameaprice;
